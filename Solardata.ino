/********************************************************************************************************
  ...
  program idea and realization by Dr. Peter Heblik , DD6USB , version 2.3 from 12/31/2021
...
//String V_String = "ver 2.3ext edited 12/31/2021 by DD6USB" ;
String V_String = "ver 2.3ext edited 13/04/2022 by OE9SAU" ;
...
void setup(void)
{
  ...
  tft.fillScreen(B_DD6USB + 3);   // clear the screen with background for the page
#ifdef WiFiOn
 ...
 // WiFi.config(ip, gateway, subnet);
  ...
  tft.println("Solar-Data@OE9SAU");
  tft.println("   by Wemos D1 mini");
  tft.println("");
  tft.println("");
  tft << "  IP: " << WiFi.localIP();
  delay(2500) ;
...
void handle_page2(void)   // first side of the solar datas
{
  ...
  tft.setFont() ;
  tft.fillScreen(B_DD6USB + 3);   // background for the page
  is_basic_pattern = false ;      // flag for page0
 // tft.fillRoundRect(0, 0, 240, 18, 5, MAROON) ;
...
void handle_page3(void)   // second side of the solar datas
{
  ...
  tft.fillScreen(B_DD6USB + 3);        // background for the page
  is_basic_pattern = false ;           // flag for page0
  //tft.fillRoundRect(0, 0, 240, 20, 3, MAROON) ;
  //tft.setCursor(180, 5) ;             // generate the top bar
  //tft.setTextColor(CYAN) ;
...
void handle_page1(void)      // side of the MUF-table
{
 ...
 // tft.setTextColor(WHITE) ;
 // tft.setCursor(3, 5) ;
 // tft.print(" loading MUF-Table from iap_kborn.de");
 // read_iap_datas() ;                            // get the muf-table-datas
  tft.fillRoundRect(0, 3, 240, 18, 5, MAROON) ; // build top header
  tft.drawRoundRect(0, 3, 240, 18, 5, WHITE) ;
  tft.setTextColor(CYAN) ;
  tft.setFont() ;
  tft.setCursor(2, 8) ;
 ...
 // tft.drawRoundRect(3, 19, 234, 298, 5, YELLOW) ; // generate the borders
  ...
  tft.drawFastVLine(120, 53, 256, YELLOW) ;      // build column-bar
  tft.drawRect(11, 50, 218, 262, YELLOW) ;       // generate table-frame
  for (g = 77 ; g < 300 ; g += 29)             // build the table-rows
  {
    tft.drawFastHLine(12, g , 216, YELLOW) ;
  }
}
//**************************************************************
...
void draw_basic_pattern(void)
{
  ...
  get_and_show_topbar_datas() ;
  draw_band_pattern() ;
  get_and_show_bottombar_datas() ;
  read_iap_datas() ;                            // get the muf-table-datas
}
//***************************************************************
void draw_band_pattern(void)
{
  ...
  tft.setFont(&FreeSans9pt7b) ;
  tft.setTextColor(YELLOW) ;
  tft.setCursor(136, 45) ;
  tft.print("Day") ;
  tft.setTextColor(YELLOW) ;
  tft.setCursor(189, 45) ;
  tft.print("Night") ;
}
//***************************************************************
...
void draw_WiFi_Quality(void)
{
  ...
  for (int8_t i = 0; i < 4; i++)
  {
    for (int8_t j = 0; j < 2 * (i + 1); j++)
    {
      if (quality > i * 25 || j == 0)
      {
        tft.drawPixel(224 + 2 * i, 12 - j, GREEN);
      }
    }
  }
}
...
void get_and_show_bottombar_datas(void)
{
 ...
 // tft.fillRoundRect(1, 302, 57, 16, 5, BLACK) ;
  ...
  //tft.print(kbuf) ;
  tft << " Flux:" << Flux << " S/N:" << SN << " MUF:" << Muf;
#ifdef debug1
  //Serial.println(kbuf) ;
  serial << "  Flux:" << Flux << " S/N:" << SN << " MUF:" << Muf;
#endif
}
//***************************************************************
...
{
  tft.setFont() ;
  tft.fillRoundRect(0, 3, 240, 18, 5, MAROON) ;
  tft.drawRoundRect(0, 3, 240, 18, 5, WHITE) ;
  tft.setTextSize(1) ;
  tft.setTextColor(WHITE) ;
  char buf1[30] ;
  ...
  //tft.print(buf1) ;
  tft << "from " << date1 << "." << date2 << " " << date3 << " " << date4 << ":" << date5 << " UTC" ;
...
{
  tft.fillRect(126, 63, 51, 35, check_day_80_40m()) ;    // 80 day
  tft.fillRect(184, 63, 52, 35, check_night_80_40m()) ;  // 80 night
  tft.fillRect(126, 103, 51, 35, check_day_30_20m()) ;   // 20 day
  tft.fillRect(184, 103, 52, 35, check_night_30_20m()) ; // 20 night
  tft.fillRect(126, 143, 51, 35, check_day_17_15m()) ;   // 15 day
  tft.fillRect(184, 143, 52, 35, check_night_17_15m()) ; // 15 night
  tft.fillRect(126, 183, 51, 35, check_day_12_10m()) ;   // 10 day
  tft.fillRect(184, 183, 52, 35, check_night_12_10m()) ; // 10 night
  tft.fillRect(184, 223, 52, 35, check_6m()) ;           //  6
  tft.fillRect(184, 263, 52, 35, check_4m()) ;           //  4
}
//********************************************************************
...
void handle_OTA(void)
{
  ...
  ArduinoOTA.setHostname("D1_mini_Solardata") ;      // Hostname defaults to esp8266-[ChipID]
